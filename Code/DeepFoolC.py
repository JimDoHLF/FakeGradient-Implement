...
def deepfoolC(image, basenet, net1, net2, net3, num_classes=10, overshoot=0.02, max_iter=50):
    ...
    while k_i == label and loop_i < max_iter:
        ...
        fsn = basenet.forward(x)
        fsn = kd.decryptKey(fsn.data.cpu())
        k_i = np.argmax(fsn.data.cpu().numpy().flatten())
        Protected = np.argmax(fsn.data.cpu().numpy().flatten()[0:1000])
